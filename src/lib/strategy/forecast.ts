...
export interface StrategicForecast {
  ...
  budgetCalc: {
    // Assuming current budget pacing continues
    dailyRunRate: number;           // avg daily spend so far this month
    remainingBudget: number;        // dailyRunRate * daysRemaining
    // Scenarios: how many leads we'll get if CPL stays / improves / worsens
    scenarios: Array<{
      label: string;              // "Current pace", "10% cheaper CPL", etc
      cpl: number;                // the CPL assumed
      newLeads: number;           // leads generated by remaining budget at that CPL
      totalLeads: number;         // MTD leads + newLeads
      vsLastMonth: number;        // totalLeads vs lastMonth.atmLeads
    }>;
  };
...
export function buildStrategicForecast(input: {
  ...
  // Pace: straight-line extrapolate MTD to month end
  const progressFactor = daysElapsedIncludingToday / daysInMonth;
  const projectedSpend = progressFactor > 0 ? Math.round((mtdSpend / progressFactor) * 100) / 100 : 0;
  const projectedATM = progressFactor > 0 ? Math.round(mtdATM / progressFactor) : 0;
  const projectedSQLs = progressFactor > 0 ? Math.round(mtdSQLs / progressFactor) : 0;
  ...
  // Budget calculator: at the current daily run-rate, how many dollars are
  // left this month? At the current CPL, how many more leads can we buy?
  const dailyRunRate = daysElapsedIncludingToday > 0 ? mtdSpend / daysElapsedIncludingToday : 0;
  const remainingBudget = Math.round(dailyRunRate * daysRemaining * 100) / 100;
